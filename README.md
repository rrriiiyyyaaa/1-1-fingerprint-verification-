...
- ~~**Verification threshold impossible** (`match_count >= 7` with only 6 samples)~~ → Fixed to `>= 4`
- ~~**`NameError` in verify** (`sample_no` undefined)~~ → Removed debug print block
- ~~**Duplicate embedding computation** in enroll~~ → Removed redundant call and debug prints
- ~~**Duplicate `import webbrowser`**~~ → Removed duplicate
- ~~**Non-deterministic model loading**~~ → Added `torch.manual_seed(42)` before model creation to ensure consistent embeddings across server restarts
